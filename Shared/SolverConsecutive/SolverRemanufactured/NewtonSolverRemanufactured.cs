...
public class NewtonSolverRemanufactured : INewtonSolver
{
    ...
    public SolverResult Solve(ISolverEquation _equation, double _alpha = 1.0)
    {
        ...
        if (nUnknowns == 0)
        {
            return new SolverResult(GetNorm(F_old) < ToleranceResidual, 0, GetNorm(F_old));
        }
        ...
        foreach (var variable in _adjustableVariables)
        {
