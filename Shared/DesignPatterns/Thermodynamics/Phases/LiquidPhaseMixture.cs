...
namespace Shared.DesignPatterns.Thermodynamics.Phases
{
    public class LiquidPhaseMixture : Phase
    {
        ...
        private void CalculateLiquidMixtureDensity()
        {
            ...
            foreach (var c in Components)
            {
                double dMass = c.PureComponentData.GetLiquidDensity(c.Temperature).GetValue(MolarDensityUnits.Kgmol_m3) * c.PureComponentData.MolecularWeight;
                if (dMass > 0) sumInvDens += c.MassFraction / dMass;
                mwMix += c.MolarFraction * c.PureComponentData.MolecularWeight;
            }
            if (sumInvDens > 0)
            {
                MassDensity = new MassDensity(1.0 / sumInvDens, MassDensityUnits.Kg_m3);
                MolarDensity = new MolarDensity(MassDensity.GetValue(MassDensityUnits.Kg_m3) / mwMix, MolarDensityUnits.Kgmol_m3);
            }
        }
