...
//    }
//    public class NewtonRaphsonSolver3
//    {
//        public int MaxIterations { get; set; } = 50;
//        public double ToleranceResidual { get; set; } = 1e-4;
//        public double ToleranceStep { get; set; } = 1e-6;
...
//        public SolverResult Solve(ISimulationSystem system, VariableDataProcedence procedence, double _alpha = 1, double _PerturbationFactor = 1e-4)
//        {
//            List<IProcessVariable> activeVariables = system.CouplingVariables.OfType<IProcessVariable>().ToList();
...
//#if DEBUG
//            Console.WriteLine($"\n[NR-Solver] 🟢 INICIANDO RESOLUCIÓN | Procedencia: {procedence}");
//            Console.WriteLine($"[NR-Solver] 📐 Ecuaciones: {nEquations} | Incógnitas: {nUnknowns}");
//#endif

//            if (nEquations != nUnknowns)
//            {
//#if DEBUG
//                Console.WriteLine($"[NR-Solver] ❌ ERROR CRÍTICO: Desbalance de grados de libertad ({nEquations} != {nUnknowns}). Abortando solver.");
//#endif
//                return new SolverResult(false, 0, double.MaxValue);
//            }
...
//            int iter = 0;
//            double alpha = _alpha;
//            PerturbationFactor = _PerturbationFactor;
//            double normF_old = GetNorm(F_old);
...
//            {
//#if DEBUG
//                Console.WriteLine($"\n[NR-Solver] 🔄 --- ITERACIÓN {iter} --- | Error Actual: {normF_old:E4}");
//#endif
//                double[,] J = CalculateJacobian(system, x_old, F_old, procedence, activeVariables);
//                double[] dx = LinearSystemSolver.Solve(J, F_old.Select(v => -v).ToArray(), SingularityTolerance);
...
//#if DEBUG
//                Console.WriteLine($"[NR-Solver] 📏 Calculando Step amortiguado (Alpha Inicial: {alpha:F4})...");
//#endif
//                var stepResult = ApplyDampedStep(system, x_old, dx, alpha, F_old, procedence, activeVariables);
...
//                    {
//                        Console.WriteLine($"[NR-Jacobian] 🚨 PELIGRO: Derivada (NaN o Infinito) detectada en Ecuación {i} al perturbar Variable '{activeVariables[j].Name}'");
//                    }
//#endif
//                }
