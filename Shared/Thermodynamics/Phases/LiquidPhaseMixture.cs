...
namespace Shared.Thermodynamics.Phases
{
    public class LiquidPhaseMixture : Phase
    {
        ...
        public void CalculateBulkProperties(Temperature temperature, Pressure pressure)
        {
            ...
            CalculateLiquidMixtureEntropy(); // 👈 NUEVO: Llamada al cálculo de entropía
        ...
        public MolarEntropy CalculateLiquidMixtureHeatCapacity(Temperature _temperature)
        {
            double cpMixMolar = 0, mwMix = 0;
            foreach (var c in Components)
            {
                cpMixMolar += c.MolarFraction * c.PureComponentData.GetLiquidHeatCapacity(_temperature).GetValue(MolarEntropyUnits.KJ_Kgmol_C);
                mwMix += c.MolarFraction * c.PureComponentData.MolecularWeight;
            }
            return new MolarEntropy(cpMixMolar, MolarEntropyUnits.KJ_Kgmol_C);
        ...
        public MolarEnergy CalculateLiquidMixtureEnthalpy(Temperature _temperature)
        {
            double hMixMolar = 0;
            double mwMix = 0;
            foreach (var c in Components)
            {
                hMixMolar += c.MolarFraction * c.PureComponentData.GetLiquidEnthalpy(_temperature).GetValue(MolarEnergyUnits.J_Kgmol);
                mwMix += c.MolarFraction * c.PureComponentData.MolecularWeight;
            }
        ...
        // ========================================================================
        // NUEVO: ENTROPÍA DE MEZCLA LÍQUIDA
        // ========================================================================
        private void CalculateLiquidMixtureEntropy()
        {
            double sMixMolar = 0;
            double mwMix = 0;
            ...
            foreach (var c in Components)
            {
                ...
                sMixMolar += c.MolarFraction * 1;// c.PureComponentData.GetLiquidEntropy(c.Temperature).GetValue(MolarEntropyUnits.J_Kgmol_C);
                mwMix += c.MolarFraction * c.PureComponentData.MolecularWeight;

                // Entropía Ideal de Mezcla: -R * sum(x_i * ln(x_i))
                if (c.MolarFraction > 0)
                {
                    sMixMolar -= R_Gas * c.MolarFraction * Math.Log(c.MolarFraction);
                }
            }
