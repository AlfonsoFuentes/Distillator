...
namespace Shared.Thermodynamics.PureComponents
{
    ...
    public class SecantSatTemperatureEvaluator : IPropertyEvaluator<Pressure, Temperature>
    {
        ...
        {
            double pTargetKpa = pressure.GetValue(PressureUnits.KiloPascala);
            double pcKpa = _criticalPressure.GetValue(PressureUnits.KiloPascala);
        ...
        private double EvaluateError(double currentTempK, double targetPressureKpa)
        {
            ...
            return pCalc.GetValue(PressureUnits.KiloPascala) - targetPressureKpa;
        }
    }
