...
namespace Shared.Results
{
    ...
    public class Result : IResult
    {
        ...
        public static IResult<T> EndPointResult<T>(T data, int result, string Succesfullymessage, string failMessage) where T : class
        {
            return result > 0 ?
                               Result<T>.Success(data, Succesfullymessage) :
                               Result<T>.Fail(failMessage);
        }
        public static IResult EndPointResult(int result, string Succesfullymessage, string failMessage)
        {
            return result > 0 ?
                               Success(Succesfullymessage) :
                               Fail(failMessage);
        }
